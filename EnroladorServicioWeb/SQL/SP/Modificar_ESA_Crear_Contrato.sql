...
BEGIN
	...
	+ ',' + '''' + @CodigoContrato + '''' + ',' + UPPER(CAST(@ConsideraAsistencia AS INT))+ ',' + UPPER(CAST(@ConsideraCasino AS INT))
	...
	END
     ...
     SELECT 
		@oid, @Empleado, @Empresa, @Cuenta, @Cargo, @InicioVigencia, @FinVigencia, 0 OptimisticLockField,NULL GCRecord,15 ObjectType,NULL Supervisor,0 TipoContrato,NULL Usuario,@ConsideraAsistencia ConsideraColacion,0 EsSupervisor, @CodigoContrato, @ConsideraCasino
