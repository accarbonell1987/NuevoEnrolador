...
namespace EnroladorStandAloneV2.Herramientas.Huellero {
    ...
    public class Huellero : IDisposable {
        ...
        private List<Huella> FingerPrintListHuella;
        ...
        public bool Connect(List<Huella> huellasUsuario) {
            if (!connected) {
                //FPTable = new Dictionary<int, string>();
                FingerPrintListHuella = new List<Huella>();
                ...
                foreach (var huella in huellasUsuario) {
                    int FPID = int.Parse(huella.GuidHuella) + 1;
                    if (FPTable.ContainsKey(FPID)) {
                        continue;
                    }
#if (DEBUG && DESACTIVAHUELLERO)
                    FPTable[FPID] = huella.Data;
#else
                    if (ZKFPEng.AddRegTemplateStrToFPCacheDB(FPCache, FPID, huella.Data) != 1) {
                        Clean();
                        return false;
                    } else {
                        FPTable[FPID] = huella.Data;
        ...
        public bool Refrescar(List<Huella> huellasUsuario) {
            if (!connected) {
                return false;
            }
            ...
            foreach (var huella in huellasUsuario) {
                ...
                if (FPTable.ContainsKey(FPID)) {
                    string value = FPTable[FPID];
                    if (value.Equals(huella.Data)) {
                        FPTable_new[FPID] = huella.Data;
                        continue;
                    }
                    if (ZKFPEng.RemoveRegTemplateFromFPCacheDB(FPCache, FPID) != 1) {
                        Disconnect();
